...
public class BattleUnit extends GameObject {
    ...
    int maxAttackRange;
    int moveRange = 3;
	int buttonTimeoutVal = 0;
	int buttonTimeoutMax = 10;
    //52x52 sprites
    //repair at buildings
    //heal?
    //combine
...
//			{
//				if(mc.X >= this.x && mc.Y >= this.y && mc.X < this.x + TerrainTile.SPRITE_SIZE && mc.Y < this.y + TerrainTile.SPRITE_SIZE)
//				{
//					TerrainTile.selectedTile.occupyingUnit = this;
...
//		}
    ...
    public boolean render(RenderContainer rc) {
		...
		if(TerrainTile.selectedTile != null && TerrainTile.selectedTile.occupyingUnit == this)
		{
			//show buttons for commands (move, attack, cancel)
			rc.LowerDraw.add(new DrawShape(DrawShape.Shape.LINE, Color.GREEN, x, y, x + TerrainTile.SPRITE_SIZE, y, 10, 0));
			rc.LowerDraw.add(new DrawShape(DrawShape.Shape.LINE, Color.GREEN, x, y, x, y + TerrainTile.SPRITE_SIZE, 10, 0));
			
			
		}
		
		rc.LowerDraw.add(new DrawShape(DrawShape.Shape.RECTANGLE, Color.BLUE, x+1, y+1, TerrainTile.SPRITE_SIZE - 2, TerrainTile.SPRITE_SIZE - 2, 0, 0));
