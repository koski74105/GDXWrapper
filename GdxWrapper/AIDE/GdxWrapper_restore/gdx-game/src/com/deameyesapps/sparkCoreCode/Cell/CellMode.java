...
public class CellMode extends GameMode
{
    ...
    public boolean mouseClick(PerspectiveCamera camera, int X, int Y) {
        mainCharacter.mouseClick(camera, X, Y);
        if(mainMenuButon.mouseClick(camera, X, Y))
        {
            ChangeMode = true;
        ...
        if(cellEditorButon.mouseClick(camera, X, Y))
        {
            ChangeMode = true;
    ...
    public void render(PerspectiveCamera camera, List<ModelInstance> modelInstances, List<DrawShape> LowerDraw, List<DrawText> text, List<DrawShape> upperDraw) {
        ...
        mainCharacter.render(camera, modelInstances, LowerDraw, text, upperDraw);

        otherChar.render(camera, modelInstances, LowerDraw, text, upperDraw);

        mainMenuButon.render(camera, modelInstances, LowerDraw, text, upperDraw);
        cellEditorButon.render(camera, modelInstances, LowerDraw, text, upperDraw);

        for(int i = 0; i < roomObjects.size();i++)
        {
            GameObject temp = roomObjects.get(i);
            temp.render(camera, modelInstances, LowerDraw, text, upperDraw);
        }
    }
