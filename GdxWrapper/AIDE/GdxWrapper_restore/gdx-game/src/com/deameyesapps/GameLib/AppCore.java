...
public class AppCore implements ApplicationListener {
    ...
    public void render() {
        ...
        if(mode != null)
            mode.render(camera, modelInstances, LowerDraw, TextDraw, UpperDraw);
        ...
        for (int i = 0; i < 20; i++)
        {
            if (Gdx.input.isTouched(i))
            {
                ...
                mode.mouseClick(camera, Gdx.input.getX(i), Gdx.graphics.getHeight() - Gdx.input.getY(i));
                clicks.add(new MouseClick(Gdx.input.getX(i), Gdx.graphics.getHeight() - Gdx.input.getY(i), i));
            }
        }

        //non3d Layers:
        mode.checkClicks(clicks);
        if(!clicked)
        {
            mode.mouseRelease();
        }
